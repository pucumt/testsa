...
function search(p) {
    ...
    $.post("/admin/absentStudentsList/search?" + pStr, filter, function(data) {
        if (data && data.absentStudentss.length > 0) {
            data.absentStudentss.forEach(function(absentStudents) {
                var $tr = $('<tr id=' + absentStudents._id + '><td>' + absentStudents.name + '</td><td>' +
                    absentStudents.sCount + '</td><td>' + absentStudents.schoolArea + '</td><td><div class="btn-group">' + getButtons() + '</div></td></tr>');
                $tr.find(".btn-group").data("obj", absentStudents);
...
$("#btnSave").on("click", function(e) {
    ...
    {
        var postURI = "/admin/absentStudents/add",
            postObj = {
            name: $.trim($('#name').val()),
            address: $.trim($('#address').val())
        };
        if (!isNew) {
            postURI = "/admin/absentStudents/edit";
            postObj.id = $('#id').val();
        }
...
$("#gridBody").on("click", "td .btnDelete", function(e) {
    ...
    $("#btnConfirmSave").off("click").on("click", function(e) {
        $.post("/admin/absentStudents/delete", {
            id: entity._id
        }, function(data) {
